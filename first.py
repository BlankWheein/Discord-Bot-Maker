...
class Main(commands.Cog):
    ...
    class cake:
        ...
        async def parseMessage(self, message : str):
            ...
            for variable in self.commandsVar:
                for attribute in [attribute for attribute in dir(self.commandsVar[variable]) if not attribute.startswith('__')]:
                    try:
        ...
        async def sendMessage(self, action):
            ...
            if not "delete_after" in action: delete_after = None
            else: delete_after = action["delete_after"]
            msg = None
    ...
    async def on_ready(self):
        ...
        if "on_ready" in self.commands:
            await self.cake(None, "on_ready", self).processCommands()

    @commands.Cog.listener()
    async def on_message_delete(self, message):
        self.commands = json.load(open("commands.json", "r"))
        if "on_message_delete" in self.commands:
            await self.cake(None, "on_message_delete", self, message=message).processCommands()
